...
export default async function TestsBlockComponent() {
  const t = await getTranslations('AvailableTests')
  ...
  return (
    <section className='relative mx-auto flex max-w-7xl flex-col items-center px-4 py-12'>
      <div className='absolute top-0 -left-1/2 -z-0 h-full w-[200vw] bg-[#F6FBFF]' />
      ...
      <div className='z-2 grid w-full gap-6 pt-8 sm:grid-cols-1 sm:grid-rows-4 md:grid-cols-2 md:grid-rows-2 lg:grid-cols-4 lg:grid-rows-1'>
        {tests.map((test, index) => {
          const title = t(`tests.${test.key}.title`)
          const duration = t(`tests.${test.key}.duration`)
          const questions = t(`tests.${test.key}.questions`)
          const buttonText = t(`tests.${test.key}.buttonText`)
            ...
            >
              <div className='mb-4 flex flex-col items-start gap-2'>
                ...
                <p className='mt-2 flex items-center gap-3 text-sm text-gray-600'>
                  <SmallClockIcon /> {duration} • <PassedIcon /> {questions}
                </p>
              </div>
              <CustomButton
                className={`mt-auto ${
